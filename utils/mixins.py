...
class SortMixin:
    ...
    def get_default_sort_params(self):
        if self.default_sort_params is None:
            raise ImproperlyConfigured("'SortMixin' requires the 'default_sort_params' attribute " "to be set.")
    ...
    def get_queryset(self):
        return self.sort_queryset(super().get_queryset(), *self.get_sort_params())
...
class FilterMixin:
    ...
    def get_context_data(self, *args, **kwargs):
        context = super().get_context_data(*args, **kwargs)
        context.update({'filter': self.get_filter_param()})
        return context
